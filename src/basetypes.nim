...
type
  ResolvedRegProperties* = object
    size*: Natural
    access*: SvdRegisterAccess
    resetValue*: int64

type
  SvdRegisterNodeKind* = enum
    rnkCluster
    rnkRegister
...
type
  SvdRegisterTreeNode* = ref object
    id*: SvdId
    name*: string
    baseName*: string
    derivedFrom*: Option[string]
...
func split*(id: SvdId): seq[string] =
  id.parts
...
func update*(base, with: SvdRegisterProperties): SvdRegisterProperties =
  result = base
  if with.size.isSome:
    result.size = with.size
  if with.access.isSome:
    result.access = with.access
  if with.resetValue.isSome:
    result.resetValue = with.resetValue

func update*(base, with: SvdDimElementGroup): SvdDimElementGroup =
  result = base
  if with.dim.isSome:
    result.dim = with.dim
  if with.dimIncrement.isSome:
    result.dimIncrement = with.dimIncrement
  if with.dimName.isSome:
    result.dimName = with.dimName
